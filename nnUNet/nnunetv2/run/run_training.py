...
from nnunetv2.training.nnUNetTrainer.nnUNetTrainer_semisup import nnUNetTrainer
...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                    ...
                                    specified_cfg=specified_cfg)
    return nnunet_trainer


def get_multi_trainer_from_args(dataset_name_or_id: Union[int, str],
                          configuration: str,
                          fold: int,
                          tasks,
                          tags,
    ...
    dataset_jsons = {}
    for task in tasks:
        preprocessed_dataset_folder_base = join(nnUNet_preprocessed, maybe_convert_to_dataset_name(task))
        plans_file = join(preprocessed_dataset_folder_base, plans_identifier + '.json')
        plans = load_json(plans_file)
        dataset_json = load_json(join(preprocessed_dataset_folder_base, 'dataset.json'))
        plans_files[task] = plans
        dataset_jsons[task] = dataset_json
    # print("dat",dataset_jsons)
    nnunet_trainer = nnunet_trainer(plans=plans_files, configuration=configuration, fold=fold, tasks=tasks, tags=tags,
                                    dataset_json=dataset_jsons, unpack_dataset=not use_compressed, device=device, specified_cfg=specified_cfg)
    return nnunet_trainer
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        print(trainer_class_name)
        print(specified_cfg)
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, trainer_class_name, specified_cfg,
...
def run_multitraining(dataset_name_or_id: Union[str, int],
                 configuration: str, fold: Union[int, str],
                 trainer_class_name: str = 'nnUNetTrainer',
                 plans_identifier: str = 'nnUNetPlans',
                 pretrained_weights: Optional[str] = None,
    ...
    for i, task in enumerate(tasks):
        ...
        if not task.startswith("Task"):
            task_id = int(task)
            task = maybe_convert_to_dataset_name(task_id)
        tasks[i] = task #Dataset400/Dataset403

        json_file = join(nnUNet_preprocessed, task, "dataset.json")
        classes = []
        ...
        for i in tags.values():
            ...
            if not int(i) == 0:  # bkg not in tag
                # print()
                # print(tags.values())
                # print(list(tags.values())[i])
                classes.append(list(tags.keys())[i])
        classes_dict[task] = classes
    # print(classes_dict)
    # exit()
    if num_gpus > 1:
        ...
        nnunet_trainer = get_multi_trainer_from_args(dataset_name_or_id, configuration, fold, tasks, classes_dict, trainer_class_name, specified_cfg,
                                               plans_identifier, use_compressed_data, device=device)
...
def run_training_entry():
    ...
    parser.add_argument('-sp', type=str, required=False, default='')
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 device=device, specified_cfg=args.sp)
    # run_multitraining(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
    #              args.num_gpus, args.use_compressed, args.npz, args.c, args.val, args.disable_checkpointing,
    #              device=device, specified_cfg=args.sp)
